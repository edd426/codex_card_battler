...
class Game {
  ...
  playCard(cardId) {
    ...
    // Spell cards: one-time effect, then discarded
    if (card.spell) {
  ...
  attack(attackerId, targetType, targetId) {
    ...
    if (targetType === 'hero') {
      // Deal damage to AI hero and apply lifesteal
    ...
    if (targetType === 'creature') {
      ...
      // Damage target (consider Divine Shield)
      ...
      // Retaliation (consider Divine Shield)
      ...
      // Lifesteal for target on retaliation
  ...
  endTurn() {
    ...
    while (true) {
      ...
      // Spell cards: one-time effect for AI
      if (card.spell) {
